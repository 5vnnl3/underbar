(function() {
  ...
  _.uniq = function(array) {
    ...
    for (var index = 0; index < array.length; index++) {
      if (_.indexOf(unique, array[index]) === -1) {
  ...
  _.every = function(collection, iterator) {
    return _.reduce(collection, function(every, element) {
      if (typeof iterator !== 'function' && element) {
        return every;
      } else if (typeof iterator === 'function' && iterator(element)) {
        return every;
      }
      return false;
    }, true);
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      var alreadyCalled = _.some(prevResults, function(results) {
        return (JSON.stringify(args) === JSON.stringify(results.args)) ? true : false;
      });
      if (alreadyCalled) {
        return prevResults[args];
      } else {
        var newResults = {};
        newResults.args = args;
        newResults.results = func.apply(this, arguments);
